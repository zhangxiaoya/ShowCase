...
void Defog::NewFindA(int *val)
{
    ...
    val[0] = std::min(220, val[0]);
    val[1] = std::min(220, val[1]);
    val[2] = std::min(220, val[2]);
}
...
void Defog::NewFindTx(int r,int *val, float w)
{
    for (auto i = 0; i < resizeHeight; ++i)
    {
        ...
        for(auto j = 0; j < resizeWidth; ++j)
        {
            ...
            ++srcPtr;
            min = std::min(min, 1.0f * (*srcPtr) / val[1]);
            ++srcPtr;
            *tempTxPtr++ = std::min(min, 1.0f * (*srcPtr) / val[2]);
            ++srcPtr;
        }
    }
